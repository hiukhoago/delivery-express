...
const Activity = (props: any) => {
    ...
    const handleFetchData = async () => {
        if (authentication) {
            await fetchData('/activity', {}, query)
                .then((res) => {
                    if (res?.status === 200) {
    ...
    const handlePatchState = async (id: any,dto : Activity ,s: ActivityState,index:any) => {
        dto.state = s
        await fetchData(`/activity/${id}`, { method: 'PATCH', data:dto}).then((res: any) => {
            if (res?.status < 300) {
                activities[index].state = s
            }
        }).catch(error => {
    ...
    const handleAcceptAll = async () => {
        setIsLoading(true)
        await fetchData('/order/accept/all', { method: 'PUT' })
            .then((res) => {
                                ...
                                <table className='text-xs w-auto min-w-full rounded border-collapse border'>
                                    <thead className='font-medium text-sm leading-4'>
                                        <tr>
                                            ...
                                            <th className='p-3 bg-gray-100 box-border relative min-w-[150px] text-left'>User id</th>
                                    ...
                                    <tbody>
                                        {activities && activities.map((n, i) => (
                                            <tr key={i}>
                                                ...
                                                {n?.state === ActivityState.REQUEST ? (
                                                    <td className='flex gap-1 items-center'>
                                                        <button onClick={() => {handlePatchState(n?.id,n,ActivityState.COMPLETE,i)}} className={'p-2 rounded-lg hover:opacity-70 ' + getStateLabelBg(ActivityState.COMPLETE).color}>Xác nhận</button>
                                                        <button onClick={() => {handlePatchState(n?.id,n,ActivityState.CANCEL,i)}} className={'p-2 rounded-lg hover:opacity-70 ' + getStateLabelBg(ActivityState.CANCEL).color}>Từ chối</button>
                                                    </td>
                                                ) : (
